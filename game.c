...
int main(){
    int frame_diamante = 0, frame_player = 0, frame_explosao = 0, counter = 1, i = 1;
    jogador player;
    unsigned char key[ALLEGRO_KEY_MAX];
    tile **area;// = aloca_area(DEFAULT_WIDTH,DEFAULT_HEIGHT);
    nodo *pedras = inicializa_lista();
    nodo *diamantes = inicializa_lista();
    nodo *butterflies = inicializa_lista();
    nodo *fireflies = inicializa_lista();
    ...
    while(1){ // menu
        ...
        al_draw_text(font, al_map_rgb(0, 255, 255), width/2, 10, ALLEGRO_ALIGN_CENTRE, "boder dash");
        ...
        al_draw_text(font, al_map_rgb(255,255,255), width/2, height/2, ALLEGRO_ALIGN_CENTER, "Pressione a barra de espaço para jogar");
        al_flip_display();
    ...
    while(1){ // game loop
        ...
        while(player.vivo && !passou){ // game loop
            ...
            switch(event.type){
                case ALLEGRO_EVENT_TIMER:
                    ...
                    if(event.timer.source == timer_pedras){
                        if(atualiza_objetos(&pedras, area, sprites, 'r'))
                    ...
                    if (key[ALLEGRO_KEY_PGDN] && jogo.n_level > 0){
                        jogo.n_level--;
                        player.vivo = 0;
                    }
            ...
            if(tela_ajuda){
                ...
                al_draw_text(font, al_map_rgb(255,255,255), width/2, 5, ALLEGRO_ALIGN_CENTER, "Ajuda");
